...
function sessionStatus(session: SessionItem): string {
  if (session.verifyCommand?.status === 'failed') return 'failed verify';
  if (session.applyResult?.success) return 'applied';
  return session.executionMode ?? 'answer';
}

function statusClassName(status: string): string {
  if (status === 'failed verify') return 'border-ember/30 bg-ember/10 text-ember';
  if (status === 'applied') return 'border-mint/30 bg-mint/10 text-mint';
  ...
  return (
    <div className="mt-6 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-skyglass">History</p>
          <h3 className="mt-3 font-display text-2xl font-semibold text-white">Local Session History</h3>
          <p className="mt-2 text-sm text-slate-500">Session history is stored locally on this computer.</p>
        </div>
      ...
      <div className="mt-5 grid gap-3">
        {sessions.length ? (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                    ...
                    {session.patchPlanSummary ? <span>{session.patchPlanSummary.changedFiles.length} changed files</span> : null}
                  </div>
                </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-ink/40 p-4 text-sm text-slate-500">
            {activeProjectId ? 'No sessions saved yet.' : 'Open a project folder to load local history.'}
          </div>
        )}
      </div>
      ...
      {activeSession ? (
        <div className="mt-5 rounded-3xl border border-white/10 bg-ink/40 p-4 text-sm text-slate-300">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h4 className="font-display text-lg font-semibold text-white">Selected Session</h4>
          ...
          <div className="mt-4 grid gap-4">
            <section>
              <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Raw request</div>
              <p className="mt-2 whitespace-pre-wrap leading-6 text-slate-200">{activeSession.rawRequest || 'No request saved.'}</p>
            </section>

            {activeSession.detectedIntent ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Detected intent</div>
            ...
            {activeSession.promptVariants?.length ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Prompt variants</div>
                <div className="mt-2 text-slate-400">{activeSession.promptVariants.length} variants saved.</div>
              </section>
            ) : null}

            {activeSession.selectedVariant ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Selected prompt</div>
            ...
            {activeSession.loadedContextFilePaths?.length ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Loaded context paths</div>
            ...
            {activeSession.executionResult ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Execution result</div>
                <pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap rounded-2xl border border-white/10 bg-ink/70 p-3 text-xs leading-5 text-slate-300">
                  {activeSession.executionResult}
                </pre>
              </section>
            ...
            {activeSession.patchPlanSummary ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Patch summary</div>
                <p className="mt-2 text-slate-200">{activeSession.patchPlanSummary.summary}</p>
                <div className="mt-2 text-xs text-slate-500">
                  {activeSession.patchPlanSummary.riskLevel} risk, {activeSession.patchPlanSummary.changedFiles.length} changed files
                </div>
              </section>
            ) : null}
            ...
            {activeSession.applyResult ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Apply result</div>
            ...
            {activeSession.verifyCommand ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Verify command</div>
                <div className="mt-2 text-slate-200">{activeSession.verifyCommand.command}</div>
                <div className="mt-1 text-xs text-slate-500">
                  {activeSession.verifyCommand.status}, exit {activeSession.verifyCommand.exitCode ?? 'n/a'}
                  {activeSession.verifyCommand.truncated ? ', output truncated' : ''}
                </div>
              </section>
            ) : null}
            ...
            {activeSession.errorAnalysis ? (
              <section>
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Error analysis</div>

  ...
  return (
    <div className="mt-6 rounded-[2rem] border border-skyglass/20 bg-white/[0.04] p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-skyglass">Xem trước patch</p>
          <h3 className="mt-3 font-display text-2xl font-semibold text-white">{patchPlan.summary}</h3>
        </div>
        <span className={`rounded-full border px-4 py-2 text-sm font-bold ${riskClassName(patchPlan.riskLevel)}`}>{patchPlan.riskLevel} rủi ro</span>
      </div>
      ...
      {!canApply ? <div className="mt-3 text-sm text-slate-500">Chỉ có thể áp dụng khi patch có tệp và không có cảnh báo chặn xác thực.</div> : null}
      ...
      {lastApplyResult ? (
        <div className="mt-5 rounded-3xl border border-mint/20 bg-mint/10 p-4 text-sm text-slate-300">
          ...
          <div className="mt-2 text-xs text-slate-500">Sao lưu: {lastApplyResult.backupId}</div>
          <div className="mt-1 text-xs text-slate-500">Áp dụng lúc: {new Date(lastApplyResult.appliedAt).toLocaleString()}</div>
      ...
      {rollbackResult ? (
        <div className="mt-4 rounded-2xl border border-skyglass/20 bg-skyglass/10 p-4 text-sm text-slate-300">
          <div className="font-semibold text-white">Hoàn tác {rollbackResult.success ? 'hoàn tất' : 'kết thúc với lỗi'}</div>
          <div className="mt-3 grid gap-2">
      ...
      <div className="mt-5 grid gap-5">
        {patchPlan.files.length ? (
          patchPlan.files.map((file) => (
            <article key={file.relativePath} className="rounded-3xl border border-white/10 bg-ink/40 p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button type="button" onClick={() => onApplyFile(file.relativePath)} disabled={!canApply || applyLoading} className="rounded-full border border-mint/30 px-3 py-2 text-xs font-bold text-mint transition hover:bg-mint/10 disabled:opacity-40">
                  Áp dụng tệp
                </button>
                <button type="button" onClick={() => onRejectFile(file.relativePath)} className="rounded-full border border-ember/30 px-3 py-2 text-xs font-bold text-ember transition hover:bg-ember/10">
                  Từ chối tệp
                </button>
                <button type="button" onClick={() => onCopyFileContent(file.relativePath)} className="rounded-full border border-white/10 px-3 py-2 text-xs font-bold text-white transition hover:border-skyglass/50">
                  Sao chép tệp mới
                </button>
              </div>
              <div className="mt-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-ink/50 p-4 text-sm text-slate-400">Không có thay đổi tệp nào được tạo.</div>
        )}
      </div>
      ...
      {isConfirmOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-panel p-6 shadow-glow">
            <h3 className="font-display text-2xl font-semibold text-white">Áp dụng thay đổi?</h3>
            <p className="mt-3 text-sm leading-6 text-slate-400">
              Thao tác này sẽ sửa {patchPlan.files.length} tệp trong dự án. Bản sao lưu sẽ được tạo trước.
            </p>

    ...
    <div>
      <button
        ...
        onDragStart={(event) => {
          ...
          event.dataTransfer.setData(
            "application/x-doni-project-file",
            node.path,
          );
      ...
      >
        ...
        <span
          className={isFile ? "truncate font-mono" : "truncate font-semibold"}
        >
          {node.name}
        </span>
      </button>
...
export function Sidebar({
  ...
  const rootChildren = Array.from(tree.children.values()).sort(
    (a, b) =>
      Number(Boolean(a.file)) - Number(Boolean(b.file)) ||
      a.name.localeCompare(b.name),
  );
  const codexStatus = useProjectStore((state) => state.codexStatus);
  const codexStatusLoading = useProjectStore(
    (state) => state.codexStatusLoading,
  );
  const refreshCodexStatus = useProjectStore(
    (state) => state.refreshCodexStatus,
  );
  ...
  const tokenText =
    typeof codexStatus?.totalTokens === "number"
      ...
      : null;
  const contextText =
    typeof codexStatus?.contextWindowTokens === "number"
      ? codexStatus.contextWindowTokens.toLocaleString()
      : null;
  ...
  useEffect(() => {
    ...
    void updater
      ...
      .catch(() => undefined);
    return updater.onStatus(setUpdaterStatus);
  }, []);
      ...
      setRefreshTreeError(
        error instanceof Error
          ? error.message.replace(
              /^Error invoking remote method 'project:scan-folder': Error: /,
              "",
            )
  ...
  return (
    <aside
      className="flex h-full shrink-0 flex-col border-r border-white/10 bg-panel/95"
      style={{ width }}
    >
        ...
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`grid h-9 w-9 place-items-center rounded-xl border text-sm font-bold transition ${
                activeView === "workspace"
                  ...
                  : "border-white/10 text-slate-400 hover:border-mint/40 hover:text-mint"
              }`}
            >
              D
            </button>
            <button
              ...
              title="Cài đặt"
              className={`relative grid h-9 w-9 place-items-center rounded-xl border text-lg transition ${
                activeView === "settings"
                  ? "border-skyglass/40 bg-skyglass/10 text-skyglass"
                  : "border-white/10 text-slate-400 hover:border-skyglass/40 hover:text-skyglass"
              }`}
            >
              ⚙
              {updaterStatus?.phase === "available" ||
              updaterStatus?.phase === "downloaded" ? (
                <span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-mint shadow-glow" />
              ) : null}
            </button>
          </div>
        </div>
      ...
      <div className="space-y-3 border-b border-white/10 p-4">
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
            ...
            <p className="mt-2 font-display text-3xl font-semibold text-mint">
              {fileCount}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
            ...
            <p className="mt-2 truncate text-sm font-semibold text-skyglass">
              {projectSummary?.frameworks.slice(0, 2).join(", ") || "n/a"}
            </p>
          </div>
        </div>
      </div>
      ...
      {projectSummary ? (
        <div className="border-b border-white/10 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {[...projectSummary.technologies, ...projectSummary.frameworks]
              .slice(0, 8)
              .map((item) => (
                <span
                  key={item}
                  className="rounded-full border border-white/10 bg-ink/60 px-2 py-1 text-xs text-slate-300"
                >
                  {item}
                </span>
              ))}
      ...
      <div className="border-b border-white/10 p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded-full border border-white/10 px-2 py-1 text-xs font-bold text-slate-300 hover:border-mint/50"
            >
              {codexStatusLoading ? "..." : "Làm mới"}
            </button>
            <button
              type="button"
              onClick={() => void probeCodexStatus()}
              className="rounded-full border border-mint/30 px-2 py-1 text-xs font-bold text-mint hover:bg-mint/10"
            >
              Kiểm tra
            </button>
          </div>
        ...
        <div
          className={`mt-3 rounded-2xl border p-3 ${codexStatus?.available ? "border-mint/20 bg-mint/10" : "border-ember/20 bg-ember/10"}`}
        >
          <div className="flex items-center justify-between gap-3">
            <span
              className={
                codexStatus?.available
                  ? "text-sm font-bold text-mint"
                  : "text-sm font-bold text-ember"
              }
            >
            ...
            <span className="text-xs text-slate-500">
              {codexStatus?.version ?? "chưa phát hiện"}
            </span>
          </div>
          <div className="mt-3">
          ...
          <div className="mt-3 space-y-1 text-xs leading-5 text-slate-500">
            {codexStatus?.lastDurationMs ? (
              <div>
                Thời lượng: {(codexStatus.lastDurationMs / 1000).toFixed(1)}s
              </div>
            ) : null}
            {tokenText ? (
              <div>
            ...
            {codexStatus?.lastModel ? (
              <div>Model: {codexStatus.lastModel}</div>
            ) : null}
          </div>
          {codexStatus?.error ? (
            <div className="mt-2 rounded-xl bg-ember/10 p-2 text-xs text-ember">
              {codexStatus.error}
            </div>
          ) : null}
        </div>
